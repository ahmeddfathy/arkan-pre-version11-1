...
@section('content')
<div class="container">
    <h2 class="page-title">{{ __('إنشاء اجتماع جديد') }}</h2>
        ...
        <div class="arkan-container">
            ...
            <form method="POST" action="{{ route('meetings.store') }}">
                ...
                <div class="arkan-form-group">
                    ...
                    <input id="title" class="arkan-form-control" type="text" name="title" value="{{ old('title') }}" required autofocus placeholder="أدخل عنوان الاجتماع..." />
                    @error('title')
                ...
                <div class="arkan-form-group">
                    ...
                    <textarea id="description" name="description" class="arkan-form-control" rows="3" placeholder="أدخل وصف الاجتماع (اختياري)...">{{ old('description') }}</textarea>
                    @error('description')
                        <p class="text-danger mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <!-- تنبيه للموظفين العاديين -->
                @if(!$hasDirectPermission)
                <div class="alert alert-warning client-approval-notice hidden">
                    <i class="fas fa-exclamation-triangle"></i>
                    <strong>⚠️ ملاحظة مهمة:</strong> اجتماعات العملاء التي تنشئها ستحتاج إلى موافقة من الـ <strong>Technical Support</strong> قبل التأكيد. سيتم إرسال طلب موافقة لمراجعة الموعد والموافقة عليه.
                </div>
                @else
                <div class="alert alert-success client-approval-notice hidden">
                    <i class="fas fa-check-circle"></i>
                    <strong>✅ معلومة:</strong> اجتماعات العملاء التي تنشئها ستكون <strong>معتمدة تلقائياً</strong> بدون الحاجة للموافقة.
                </div>
                @endif

                @if($canViewClients)
                <div class="arkan-form-group client-field hidden">
                ...
                <div class="arkan-form-group project-field hidden">
                ...
                <div class="row">
                    <div class="col-md-6">
                        <div class="arkan-form-group">
                            ...
                            <input id="start_time" class="arkan-form-control" type="datetime-local" name="start_time" value="{{ old('start_time') }}" required />
                            @error('start_time')
                                <p class="text-danger mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                    ...
                    <div class="col-md-6">
                        <div class="arkan-form-group">
                            ...
                            <input id="end_time" class="arkan-form-control" type="datetime-local" name="end_time" value="{{ old('end_time') }}" required />
                            @error('end_time')
                                <p class="text-danger mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                ...
                <div class="arkan-form-group">
                    ...
                    <input id="location" class="arkan-form-control" type="text" name="location" value="{{ old('location') }}" placeholder="أدخل مكان الاجتماع (اختياري)..." />
                    @error('location')
                        <p class="text-danger mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="text-end mt-4">
                    <a href="{{ route('meetings.index') }}" class="arkan-btn-secondary">
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (clientSelect) {
            ...
            // Event listener for project selection (auto-select client) - only if client select exists
