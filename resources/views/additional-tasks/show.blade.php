...
<div class="additional-tasks-container" style="width: 100%; padding: 0 2rem;">
                                ...
                                <tr>
                                    ...
                                    <td class="text-sm font-medium">
                                        <div class="flex gap-2">
                                            ...
                                            @elseif($taskUser->status === 'assigned')
                                            <button onclick="showRevokeApprovalModal({{ $taskUser->id }}, '{{ addslashes($taskUser->user->name) }}')"
                                                class="btn-modern btn-warning btn-sm">
                                                <i class="fas fa-undo"></i> إلغاء الموافقة
                                            </button>
                                            @if($taskUser->user_notes || $taskUser->admin_notes)
    ...
    <div id="revokeApprovalModal" class="modal-overlay modal-overlay-hidden">
        <div class="modal-content">
            <h3 class="modal-title">
            ...
            <div class="alert-modern alert-warning mb-4">
                ...
                <span>سيتم حذف المهمة العادية المرتبطة وإرجاع الطلب إلى حالة "في الانتظار"</span>
            </div>
            <form action="#" method="POST" id="revokeApprovalForm">
                @csrf
                <input type="hidden" id="revoke_task_user_id" name="task_user_id">

                <div class="form-group">
                    <label class="form-label">الموظف</label>
                    <div id="revoke_user_name" class="text-neutral-900 font-medium"></div>
                </div>

                <div class="form-group">
                    <label for="revoke_reason" class="form-label">سبب الإلغاء (اختياري)</label>
                    <textarea id="revoke_reason" name="reason" rows="3"
                        class="form-input"
                        placeholder="اذكر سبب إلغاء الموافقة (سيتم حفظه في ملاحظات الإدارة)"></textarea>
                </div>

                <div class="modal-actions">
                    <button type="button" onclick="hideRevokeApprovalModal()" class="btn-modern btn-secondary">
                        إلغاء
                    </button>
                    <button type="submit" class="btn-modern btn-warning">
                        <i class="fas fa-undo"></i> إلغاء الموافقة
    ...
    <script>
        // Revoke Approval Modal functions - Must be global for onclick to work
        function showRevokeApprovalModal(taskUserId, userName) {
            const modal = document.getElementById('revokeApprovalModal');
            if (!modal) {
                console.error('revokeApprovalModal not found');
                return;
            }

            document.getElementById('revoke_task_user_id').value = taskUserId;
            document.getElementById('revoke_user_name').textContent = userName;
            document.getElementById('revokeApprovalForm').action = '/additional-task-users/' + taskUserId + '/revoke-approval';

            // Show modal using same method as other modals
            modal.classList.remove('modal-overlay-hidden');
            modal.classList.add('active');
        }

        function hideRevokeApprovalModal() {
            const modal = document.getElementById('revokeApprovalModal');
            if (!modal) return;

            // Hide modal using same method as other modals
            modal.classList.remove('active');
            setTimeout(() => {
                modal.classList.add('modal-overlay-hidden');
            }, 300);
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Close revoke modal when clicking outside
            const revokeModal = document.getElementById('revokeApprovalModal');
            if (revokeModal) {
                revokeModal.addEventListener('click', function(event) {
                    if (event.target === revokeModal) {
                        hideRevokeApprovalModal();
                    }
                });
            }
            ...
            window.showRevokeApprovalModal = showRevokeApprovalModal;
            window.hideRevokeApprovalModal = hideRevokeApprovalModal;
        });
    </script>