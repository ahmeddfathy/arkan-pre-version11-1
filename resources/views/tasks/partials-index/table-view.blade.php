    ...
    <table class="table align-items-center mb-0" id="tasksTable">
        ...
        <tbody>
            ...
            <tr data-task-id="{{ isset($task->task_user_id) ? $task->task_user_id : $task->id }}"
                ...
                class="{{ $task->created_by == Auth::id() ? 'my-created-task' : '' }}">
                <td>
                ...
                <td>{{ $task->project->name ?? 'غير محدد' }}</td>
                ...
                <td>
                    ...
                    @if(!in_array($displayStatus, ['completed', 'cancelled']) && !(isset($task->is_transferred) && $task->is_transferred) && (auth()->user()->hasRole('hr') || auth()->user()->hasRole('admin') || $task->created_by == Auth::id()))
