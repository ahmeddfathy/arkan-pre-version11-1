<x-app-layout>
    ...
    <x-slot name="header">
        <h2 class="h4 mb-0">إضافة حقل جديد</h2>
    </x-slot>

    <div class="container-fluid py-4">
        <div class="card shadow-sm">
            <div class="card-body">
                <form action="{{ route('project-fields.store') }}" method="POST">
                    @csrf
                    ...
                    <div class="mb-3">
                        ...
                        <select name="field_type" id="field_type" required class="form-select" onchange="toggleOptionsField()">
                            ...
                            <option value="textarea" {{ old('field_type') == 'textarea' ? 'selected' : '' }}>نص طويل</option>
                            <option value="number" {{ old('field_type') == 'number' ? 'selected' : '' }}>رقم</option>
                    ...
                    <div id="options-container" class="mb-3" style="display: {{ old('field_type') == 'select' ? 'block' : 'none' }}">
                        <label class="form-label">خيارات القائمة</label>
                        <div id="options-list">
                            @if(old('field_options'))
                                @foreach(old('field_options') as $index => $option)
                    ...
                    <!-- الترتيب -->
                    <div class="mb-3">
                        <label for="order" class="form-label">الترتيب (اختياري)</label>
                        <input type="number" name="order" id="order" class="form-control" value="{{ old('order') }}" placeholder="سيتم تحديده تلقائياً إذا تركته فارغاً">
                        @error('order')
                            <div class="text-danger small mt-1">{{ $message }}</div>
                        @enderror
                    </div>
