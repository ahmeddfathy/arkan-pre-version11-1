...
function preventButtonDeformation() {
    const uploadButtons = document.querySelectorAll(
        '.attachment-upload-form button, .upload-form button, button[type="submit"]'
    );
    ...
    uploadButtons.forEach((button) => {
        ...
        ["mousedown", "mouseup", "click", "focus", "blur", "active"].forEach(
            (event) => {
...
function addAttachmentToList(attachmentData, serviceType) {
    const attachmentList = document.querySelector(
        `[data-service-type="${serviceType}"] .list-group`
    );
    if (!attachmentList) return;

    const existingItem = attachmentList.querySelector(
        `[data-attachment-id="${attachmentData.id}"]`
    );
    ...
    const itemContent = `
        <div class="d-flex w-100 justify-content-between align-items-center">
            <a href="/projects/attachments/view/${
                attachmentData.id
            }" target="_blank" class="me-2" title="عرض الملف">
                <i class="fas fa-eye"></i>
            </a>
            <span>${attachmentData.fileName}</span>
            <div>
                <a href="/projects/attachments/download/${
                    attachmentData.id
                }" class="btn btn-sm btn-outline-primary me-1" title="تحميل">
        ...
        ${
            attachmentData.description
                ? `<small class="text-muted">${attachmentData.description}</small>`
                : ""
        }
        ${
            ...
            <small class="text-primary">
                ...
                ${
                    attachmentData.task_type === "template_task"
                        ? "قالب مهمة: "
                        : "مهمة: "
                }
        ...
        <small class="text-secondary">بواسطة: ${
...
function addFileToSelectedList(file, serviceType) {
    ...
    if (
        !selectedFilesMap[serviceType].some(
...
function updateSelectedFilesList(serviceType) {
    const selectedFilesContainer = document.getElementById(
        "selectedFiles" + serviceType.replace(/\s+/g, "")
    );
    ...
    if (
        !selectedFilesMap[serviceType] ||
        selectedFilesMap[serviceType].length === 0
    ) {
        return;
    }
    ...
    selectedFilesMap[serviceType].forEach((file, index) => {
        ...
        fileItem.innerHTML = `
            ...
            <div class="selected-file-progress" id="progress-file-${serviceType.replace(
                /\s+/g,
                ""
            )}-${index}"></div>
        `;
        ...
        fileItem
            .querySelector(".selected-file-remove")
...
function updateFileProgress(serviceType, fileIndex, percent) {
    const progressElement = document.getElementById(
        `progress-file-${serviceType.replace(/\s+/g, "")}-${fileIndex}`
    );
    const progressBar = document.getElementById(
        "progress-bar-" + serviceType.replace(/\s+/g, "")
    );
    const percentageDiv = document.getElementById(
        "percentage-" + serviceType.replace(/\s+/g, "")
    );
...
async function confirmUpload(
    ...
    try {
        const response = await fetch(
            `/projects/attachments/${attachmentId}/confirm-upload`,
            {
                method: "POST",
                headers: {
...
async function uploadMultipleFiles(formElement, serviceType, description) {
    ...
    const projectId =
        formElement.closest("[data-project-id]")?.dataset.projectId ||
    ...
    const taskType =
        formElement.querySelector(".task-type-selector")?.value || "";
    let taskId = null;
    let taskName = "";

    if (taskType === "template_task") {
        const taskSelect = formElement.querySelector(
            'select[name="template_task_user_id"]'
        );
    ...
    const progressContainer = document.getElementById(
        "progress-container-" + serviceType.replace(/\s+/g, "")
    );
    const progressBar = document.getElementById(
        "progress-bar-" + serviceType.replace(/\s+/g, "")
    );
    const statusDiv = document.getElementById(
        "status-" + serviceType.replace(/\s+/g, "")
    );
    const percentageDiv = document.getElementById(
        "percentage-" + serviceType.replace(/\s+/g, "")
    );
    ...
    for (let i = 0; i < selectedFilesMap[serviceType].length; i++) {
        ...
        try {
            ...
            const isReplyCheckbox = formElement.querySelector(
                'input[name="is_reply"]'
            );
            const parentAttachmentSelect = formElement.querySelector(
                'select[name="parent_attachment_id"]'
            );

            if (
                isReplyCheckbox &&
                isReplyCheckbox.checked &&
                parentAttachmentSelect &&
                parentAttachmentSelect.value
            ) {
            ...
            const response = await fetch(
                `/projects/${projectId}/presigned-url`,
                {
                    method: "POST",
                    headers: {
                        "X-CSRF-TOKEN": document.querySelector(
            ...
            if (!response.ok) {
                throw new Error(
                    "فشل في الحصول على رابط الرفع: " + response.status
                );
            }
            ...
            const uploadPromise = new Promise((resolve, reject) => {
                ...
                xhr.onload = function () {
                    if (xhr.status >= 200 && xhr.status < 300) {
                        ...
                        reject(
                            new Error(
                                `فشل في رفع الملف: ${xhr.status} ${xhr.statusText}`
                            )
                        );
                    }
                };
            ...
            if (uploadResponse && data.attachment_id) {
                const confirmSuccess = await confirmUpload(
                    data.attachment_id,
                    serviceType,
                    file.name,
                    description,
            ...
            const progressElement = document.getElementById(
                `progress-file-${serviceType.replace(/\s+/g, "")}-${i}`
            );
            if (progressElement) {
            ...
            toastr.error(
                `فشل في رفع الملف ${file.name}: ${error.message}`,
                "خطأ ❌"
            );
        }
    }
        ...
        if (failedFiles === 0) {
            ...
            setTimeout(() => {
                ...
                if (percentageDiv) {
                    ...
                    percentageDiv.classList.remove("show", "completed");
                }
                if (progressContainer) {
    ...
    if (uploadedFiles > 0) {
        toastr.success(
            `تم رفع ${uploadedFiles} ملف من أصل ${totalFiles} بنجاح!`,
            "اكتمل الرفع ✅"
        );
    }
}
...
function initFileUploadFunctionality() {
    ...
    uploadForms.forEach((form) => {
        const serviceTypeInput = form.querySelector(
...
function initTaskSelectionFunctionality() {
    ...
    taskTypeSelectors.forEach((selector) => {
        selector.addEventListener("change", function () {
            ...
            const templateSelector = form.querySelector(
                ".template-task-selector"
            );
            const regularSelector = form.querySelector(
                ".regular-task-selector"
            );
        ...
        checkbox.addEventListener("change", function () {
            ...
            if (this.checked) {
                ...
                if (
                    ...
                    toastr.warning(
                        "لا توجد ملفات للرد عليها في هذا المجلد",
                        "تنبيه ⚠️"
                    );
                    this.checked = false;
...
function initAttachmentClickHandlers() {
    document.addEventListener("click", function (e) {
        if (e.target && typeof e.target.closest === "function") {
            const attachmentLink = e.target.closest(
                ".attachment-name, .attachment-info a"
            );
            if (attachmentLink) {
    ...
    document.addEventListener("click", function (e) {
        if (e.target && typeof e.target.closest === "function") {
            const attachmentItem = e.target.closest(
                ".attachment-item, .list-group-item"
            );
            if (
                attachmentItem &&
                !e.target.closest(".dropdown, .btn, .form-check")
            ) {
                const attachmentLink = attachmentItem.querySelector(
                    ".attachment-name, .attachment-info a"
                );
                if (attachmentLink) {
