...
function initializeFilters() {
    ...
    $("#projectCodeFilter").on("input change", function () {
        ...
        if (enteredCode) {
            ...
            const matchingProject = $projectFilter
                .find('option[data-code="' + enteredCode + '"]')
                .first();
    ...
    $("#myCreatedTasksBtn").click(function () {
        const currentUserId =
            window.currentUserId || $('meta[name="user-id"]').attr("content");
        if (currentUserId) {
...
function filterTasks() {
    ...
    const tableSelector =
        $("#myTasksTable").length > 0
            ? "#myTasksTable tbody tr"
            : "#tasksTable tbody tr";
    $(tableSelector).each(function () {
        ...
        if (assignedUserId) {
            ...
            if (
                !assignedUsers ||
                !assignedUsers.includes(parseInt(assignedUserId))
            ) {
                show = false;
            }
        }
        ...
        if (show && (dateFrom || dateTo)) {
            const taskDate = $(this).data(
                dateType === "created_at" ? "created-at" : "due-date"
            );
    ...
    // ✅ دعم Tasks Index Calendar
    if (window.currentView === "calendar") {
        if (
            typeof window.tasksIndexCalendar !== "undefined" &&
            typeof window.tasksIndexCalendar.applyFilters === "function"
        ) {
            window.tasksIndexCalendar.applyFilters();
        }
    }

    // ✅ دعم My Tasks Kanban
    if (
        typeof window.myTasksCurrentView !== "undefined" &&
    ...
    if (
        ...
        if (
            typeof window.myTasksCalendar !== "undefined" &&
...
function filterKanbanTasks(
    ...
    const filteredTasks = allTasks.filter((task) => {
        ...
        if (createdBy) {
            ...
            const isTemplate =
                task.isTemplate === true || task.isTemplate === "true";
        ...
        if (dateFrom || dateTo) {
            const taskDate =
                dateType === "created_at" ? task.createdAt : task.dueDate;
...
function createSimpleTaskCard(task) {
    ...
    return `
        <div class="kanban-card ${isMyTask ? "my-created-task" : ""}"
            ...
            <div class="kanban-card-title">${task.name}${
        task.isTemplate
        ...
        task.revisionsCount && task.revisionsCount > 0
            ? ` <span class="task-revisions-badge ${
                  task.revisionsStatus || "pending"
              } ms-1" title="${getFilterRevisionStatusTooltip(
                  task
        ...
        task.isTransferred
            ? ' <span class="badge badge-sm bg-warning ms-1" title="تم نقل هذه المهمة"><i class="fas fa-exchange-alt"></i> تم نقلها</span>'
            : ""
    }${
        task.isAdditionalTask && task.taskSource === "transferred"
            ? ' <span class="badge badge-sm bg-success ms-1" title="مهمة منقولة إليك"><i class="fas fa-plus-circle"></i> منقولة إليك</span>'
            : ""
    }</div>
            ...
            <div class="kanban-card-meta mb-2">
                <span class="kanban-card-creator" data-creator-id="${
                    task.createdById
                }">أنشأت بواسطة: ${task.createdBy}</span>
            </div>
 ...
 */
function filterKanbanTasksFromDOM(
    projectId,
    serviceId,
    status,
    createdBy,
    ...
    $(".kanban-card").each(function () {
        ...
        const $tableRow = $(
            `#tasksTable tbody tr[data-task-id="${taskId}"]`
        ).first();
        ...
        if ($tableRow.length > 0) {
            ...
            cardProjectId =
                $card.data("project-id") || $card.attr("data-project-id");
            cardServiceId =
                $card.data("service-id") || $card.attr("data-service-id");
            cardCreatedBy =
                $card.find(".kanban-card-creator").data("creator-id") ||
                $card.data("created-by");
            cardAssignedUsers =
                $card.data("assigned-users") ||
                $card.attr("data-assigned-users");
            cardDueDate = $card.data("due-date") || $card.attr("data-due-date");
            cardCreatedAt =
                $card.data("created-at") || $card.attr("data-created-at");
        }
        ...
        if (assignedUserId) {
            if (
                !cardAssignedUsers ||
                !cardAssignedUsers.includes(parseInt(assignedUserId))
            ) {
                show = false;
            }
        }
        ...
        if (show && (dateFrom || dateTo)) {
            const taskDate =
                dateType === "created_at" ? cardCreatedAt : cardDueDate;
