...
function applyMyTasksStoredViewPreference() {
    if (myTasksCurrentView === "kanban") {
        ...
        const alreadyApplied =
            kanbanView.is(":visible") && tableView.is(":hidden");
...
function initializeMyTasksKanban() {
    ...
    window.filterTasks = function () {
        ...
        if (myTasksCurrentView === "kanban") {
            ...
            window.MyTasksKanban.filterMyTasksKanban(
                projectId,
                status,
                searchText
            );
    ...
    setInterval(function () {
        if (myTasksCurrentView === "kanban") {
            requestAnimationFrame(() => {
                $('.my-kanban-card[data-status="in_progress"]').each(
                    function () {
                        const taskId = $(this).data("task-id");
                        const timerElement = $(
                            `.task-timer[data-task-id="${taskId}"]`
                        );
                        const cardTimerElement = $(this).find(
                            ".my-kanban-card-timer"
                        );
                        if (timerElement.length && cardTimerElement.length) {
                            const currentTimer = timerElement.text().trim();
                            const currentDisplay = cardTimerElement
                                .text()
                                .trim();
                            if (currentDisplay !== currentTimer) {
                                cardTimerElement.html(
                                    `<i class="fas fa-clock"></i> ${currentTimer}`
                                );
                            }
                        }
                    }
