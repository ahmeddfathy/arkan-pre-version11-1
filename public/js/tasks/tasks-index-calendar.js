...
class TasksIndexCalendar {
    ...
    loadTasks() {
        ...
        this.tasks = [];
        const tableRows = document.querySelectorAll('#tasksTable tbody tr[data-project-id]');
        ...
        tableRows.forEach(row => {
            ...
            const createdByElement = row.querySelector('td:nth-child(4) .badge');
    ...
    createTaskElement(task) {
        ...
        taskDiv.title = `${task.name} - ${task.projectName} (${task.serviceName}) - منشئ: ${task.createdBy}`;
    ...
    openTaskDetails(task) {
        ...
        if (typeof openTaskSidebar === 'function') {
            ...
            console.log('🔍 Index Calendar view task clicked - Opening Sidebar:', {
                taskId: task.id,
                ...
                <div class="border-bottom pb-2 mb-2">
                    <div class="d-flex justify-content-between align-items-start">
                        <div style="cursor: pointer;" onclick="openTaskSidebar('${taskType}', '${taskUserId}'); Swal.close();">
                            ...
                            ${task.isMyCreated ? '<span class="badge bg-primary ms-1">مهمتي</span>' : ''}
                            <br>
                            <small class="text-muted">${task.projectName} - ${task.serviceName}</small>
                            <br>
                            <small class="text-muted">منشئ: ${task.createdBy}</small>
                            <br>
...
function initializeTasksIndexCalendar() {
    ...
    ['projectFilter', 'serviceFilter', 'statusFilter', 'createdByFilter', 'searchInput'].forEach(filterId => {
        const element = document.getElementById(filterId);
        if (element) {
            element.addEventListener('change', () => tasksIndexCalendar.applyFilters());
            element.addEventListener('input', () => tasksIndexCalendar.applyFilters());
        }
    });
