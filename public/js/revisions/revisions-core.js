...
function getCurrentReviewer(revision) {
    if (
        !revision.reviewers ||
        !Array.isArray(revision.reviewers) ||
        revision.reviewers.length === 0
    ) {
        return null;
    }

    const currentReviewer = revision.reviewers.find(
...
function getAllReviewers(revision) {
    if (
        ...
        Array.isArray(revision.reviewers_with_data)
    ) {
...
$(document).ready(function () {
    ...
    $('#revisionTabs button[data-bs-toggle="pill"]').on(
        "shown.bs.tab",
        function (e) {
    ...
    $("#allSearchInput, #mySearchInput, #myCreatedSearchInput").on(
        "keyup",
        debounce(function () {
            let tabType = "all";
            ...
            <div class="stats-card">
                ...
                <div class="row">
                    <div class="col-6">
                        <div class="stats-item">
                            <div class="stats-number">${
                                stats.my_created_revisions.total
                            }</div>
                    ...
                    <div class="col-6">
                        <div class="stats-item">
                            <div class="stats-number">${
                                stats.my_created_revisions.new || 0
                            }</div>
                            <div>في الانتظار</div>
                        </div>
                    </div>
                    <div class="col-6">
                        <div class="stats-item">
                            <div class="stats-number">${
                                stats.my_created_revisions.in_progress || 0
                            }</div>
                            <div>جاري العمل</div>
                        </div>
                    </div>
                    <div class="col-6">
                        <div class="stats-item">
                            <div class="stats-number">${
                                stats.my_created_revisions.paused || 0
                            }</div>
                            <div>متوقفة</div>
                        </div>
                    </div>
                    <div class="col-6">
                        <div class="stats-item">
                            <div class="stats-number">${
                                stats.my_created_revisions.completed || 0
                            }</div>
                            <div>مكتملة</div>
                        </div>
                    </div>
...
function groupProjectsByStatus(revisions) {
    ...
    projects.forEach((project) => {
        ...
        if (project.statuses.has("in_progress"))
...
function renderProjectsByStatusTabs(projectsByStatus) {
    const statusConfig = {
        all: {
            ...
            icon: "fa-list",
            color: "#667eea",
            count: projectsByStatus.all.length,
        },
        new: {
            label: "جديد",
            icon: "fa-plus-circle",
            color: "#6c757d",
            count: projectsByStatus.new.length,
        },
        in_progress: {
            label: "جاري العمل",
            icon: "fa-spinner",
            color: "#0d6efd",
            count: projectsByStatus.in_progress.length,
        },
        paused: {
            label: "متوقف",
            icon: "fa-pause-circle",
            color: "#ffc107",
            count: projectsByStatus.paused.length,
        },
        completed: {
            label: "مكتمل",
    ...
    tabsRow +=
        '<td colspan="8" style="padding: 0; background: #f8f9fa; border-bottom: 2px solid #e9ecef;">';
    ...
    Object.keys(statusConfig).forEach((status, index) => {
        ...
        if (projects.length === 0) {
            ...
            projects.forEach((project) => {
                ...
                tableHtml += `
                    <tr class="projects-rows-${status} ${activeClass}"
                        onclick="filterRevisionsByProject(${project.id}, '${
                    project.code
                }')"
                        ...
                        <td>
                            <button class="btn btn-sm btn-primary"
                                    onclick="event.stopPropagation(); filterRevisionsByProject(${
                                        project.id
                                    }, '${project.code}')"
                                    title="عرض التعديلات">
...
function renderRevisions(data, type) {
    ...
    const projectRevisions = data.data.filter(
        (r) => r.revision_type === "project"
    );
    ...
    const inProgressRevisions = Object.values(revisionTimers).filter(
        (timer) => timer.status === "in_progress"
    );
...
function renderRevisionRow(revision) {
    ...
    const isAssignedToMe =
        revision.assigned_to == currentUserId ||
        revision.executor_user_id == currentUserId ||
        (revision.task_user && revision.task_user.user_id == currentUserId) ||
        (revision.template_task_user &&
            revision.template_task_user.user_id == currentUserId);
    ...
    if (revision.status === "in_progress") {
        ...
        if (!revisionTimers[revision.id]) {
            ...
            console.log(
                "♻️ تحديث بيانات التعديل (بدون تغيير الـ seconds):",
                revision.id
            );
        }
    }
    ...
    if (revision.review_status === "in_progress") {
        ...
        console.log(
            "🕐 حساب الوقت الأولي للمراجعة",
            revision.id,
            ":",
            initialReviewTimerSeconds,
            "ثانية"
        );
        ...
        if (!reviewTimers[revision.id]) {
            ...
            console.log(
                "📝 إضافة المراجعة للـ review timers object:",
                revision.id
            );
        } else {
            ...
            console.log(
                "♻️ تحديث بيانات المراجعة (بدون تغيير الـ seconds):",
                revision.id
            );
        }
    }
    ...
    return `
        <tr data-revision-id="${revision.id}">
            <td onclick="showRevisionDetails(${
                revision.id
                ...
                ${
                    revision.revision_code
                        ? `
                    <small class="text-info d-block text-truncate" style="max-width: 200px; font-weight: 600; font-family: 'Courier New', monospace; font-size: 0.75rem;" title="كود التعديل: ${revision.revision_code}">
                ...
                <small class="text-muted d-block text-truncate" style="max-width: 200px;" title="${
        ...
        revision.status
            ...
            <td onclick="showRevisionDetails(${
                ...
                <div class="user-info" title="${
                    ...
                    <div class="user-avatar">
                        ${
                            revision.creator
                                ? revision.creator.name.charAt(0)
                                : "غ"
                        }
                    </div>
            ...
            <td onclick="showRevisionDetails(${
                ...
                ${
                    ...
                    <div class="user-info" title="${
                        ...
                        <div style="max-width: 100%; overflow: hidden;">
                            <div class="fw-bold small text-danger text-truncate">${
                                revision.responsible_user.name
                            }</div>
                        </div>
                    </div>
            ...
            <td onclick="showRevisionDetails(${
                ...
                ${(() => {
                    ...
                    const firstReviewerName = firstReviewer.user
                        ? firstReviewer.user.name
                        : window.allUsers?.find(
                              (u) => u.id == firstReviewer.reviewer_id
                          )?.name || `مراجع ${firstReviewer.order}`;

                    const moreCount = reviewers.length - 1;
                    const allNames = reviewers
                        .map((r) => {
                            return r.user
                                ? r.user.name
                                : window.allUsers?.find(
                                      (u) => u.id == r.reviewer_id
                                  )?.name || `مراجع ${r.order}`;
                        })
                        .join("، ");
                    ...
                    <div class="user-info" title="${allNames}">
                        ...
                        <div style="max-width: 100%; overflow: hidden;">
                            <div class="fw-bold small text-success text-truncate">
                                ${firstReviewerName}${
                        moreCount > 0
                            ? ` <span class="badge bg-success" style="font-size: 9px;">+${moreCount}</span>`
                            : ""
                    }
                            </div>
                        </div>
            ...
            <td onclick="showRevisionDetails(${
                ...
                ${
                    ...
                    <small class="text-success fw-bold">
                        <i class="fas fa-stopwatch me-1"></i>
                        ${formatRevisionTime(
                            revision.review_actual_minutes * 60
                        )}
                    </small>
                `
                        : '<small class="text-muted">-</small>'
            ...
            <td onclick="showRevisionDetails(${
                ...
                ${
                    ...
                    <div class="revision-timer" style="font-family: 'Courier New', monospace; font-weight: bold; color: #059669; padding: 2px 6px; background: #dcfce7; border-radius: 4px; font-size: 11px; text-align: center;">
                        ...
                        <span id="revision-timer-${
                            revision.id
                        }">${formatRevisionTime(
                              revisionTimers[revision.id]
                                  ? revisionTimers[revision.id].seconds
                                  : initialTimerSeconds
                          )}</span>
                    </div>
                `
            ...
            <td onclick="showRevisionDetails(${
                ...
                ${
                    ...
                    <div class="revision-timer" style="font-family: 'Courier New', monospace; font-weight: bold; color: #198754; padding: 2px 6px; background: #d1e7dd; border-radius: 4px; font-size: 11px; text-align: center;">
                        <i class="fas fa-clock"></i>
                        <span id="review-timer-${
                            revision.id
                        }">${formatRevisionTime(
                              reviewTimers[revision.id]
                                  ? reviewTimers[revision.id].seconds
                                  : initialReviewTimerSeconds
                          )}</span>
                    </div>
                `
            ...
            <td onclick="showRevisionDetails(${
                ...
                ${
                    revision.project
                        ? `
                    <small class="text-truncate d-block" style="max-width: 150px;" title="${
            ...
            <td onclick="event.stopPropagation();">
                ${
                    ...
                    <div class="d-flex gap-1 flex-wrap" style="min-width: 120px;">
                        ${
                            isAssignedToMe &&
                            ["new", "in_progress", "paused"].includes(
                                revision.status
                            )
                        ...
                        ${
                            ...
                            ["new", "in_progress", "paused"].includes(
                                ...
                                ? getReviewActionButtonsCompact(revision)
                                : ""
                        }
                        ${
...
function renderTaskRevisionRow(revision) {
    ...
    if (revision.task_user && revision.task_user.user) {
        ...
        revision.template_task_user &&
        revision.template_task_user.user
    ) {
    ...
    return `
        ...
        revision.status
            ...
            <td onclick="showRevisionDetails(${
                ...
                ${
                    ...
                    <div class="user-info" title="${assignedUser.name}">
                        ...
                        <div style="max-width: 100%; overflow: hidden;">
                            <div class="fw-bold small text-info text-truncate">${
                                assignedUser.name
                            }</div>
                        </div>
                    </div>
            ...
            <td onclick="event.stopPropagation();">
                ${
                    isAssignedToMe &&
                    ["new", "in_progress", "paused"].includes(revision.status)
                        ? `
                    <div class="d-flex gap-1 flex-wrap" style="min-width: 120px;">
                        ${getRevisionActionButtonsCompact(revision)}
                        ${
...
function refreshData(showToast = false) {
    ...
    const activeTab = $("#revisionTabs .nav-link.active").attr(
        "data-bs-target"
    );
...
function showRevisionDetails(revisionId) {
    ...
    $(`.revisions-table tbody tr[data-revision-id="${revisionId}"]`).addClass(
        "active"
    );
        ...
        .done(function (response) {
            if (response.success) {
                console.log("📅 Revision deadline:", {
                    ...
                    hasDeadline: !!(
                        response.revision.revision_deadline ||
                        response.revision.revisionDeadline
                    ),
                });
    ...
    const html = `
        <div class="detail-section">
            ...
            ${
                revision.revision_code
                    ? `
                <p><strong>كود التعديل:</strong>
                    <span class="badge bg-light text-dark" style="font-size: 0.9em; padding: 0.4em 0.8em; letter-spacing: 0.5px; font-family: 'Courier New', monospace; border: 1px solid #dee2e6;">
                        <i class="fas fa-barcode me-1"></i>
                        ${revision.revision_code}
                    </span>
                </p>
            ...
            <p><strong>النوع:</strong> ${getRevisionTypeText(
                revision.revision_type
            )}</p>
            <p><strong>المصدر:</strong> ${getSourceText(
                revision.revision_source
            )}</p>
            <p><strong>الحالة:</strong>
                <span class="status-badge status-${
                    revision.status
                }">${getStatusText(revision.status)}</span>
            </p>
            ...
            ${
                revision.actual_minutes
                    ? `
                <p><strong>الوقت المستغرق:</strong> ${formatRevisionTimeInMinutes(
                    revision.actual_minutes
                )}</p>
            `
                    : ""
            ...
            ${(() => {
                const revisionDeadline =
                    revision.revision_deadline || revision.revisionDeadline;
                if (revisionDeadline) {
                    ...
                    if (diffMs < 0) {
                        ...
                        deadlineText = `⚠️ التعديل متأخر بـ ${Math.abs(
                            diffDays
                        )} يوم`;
                        deadlineBg = "#fef2f2";
                    ...
                    try {
                        ...
                        const hours = deadlineDate
                            .getHours()
                            .toString()
                            .padStart(2, "0");
                        const minutes = deadlineDate
                            .getMinutes()
                            .toString()
                            .padStart(2, "0");
        ...
        ${
            isCurrentUserReviewer(revision) &&
            ["new", "in_progress", "paused"].includes(revision.review_status)
                ? `
            <div class="detail-section bg-success bg-opacity-10">
                ...
                ${
                    revision.review_actual_minutes
                        ? `
                    <p class="mt-2 mb-0"><strong>وقت المراجعة:</strong> ${formatRevisionTimeInMinutes(
                        revision.review_actual_minutes
                    )}</p>
                `
                        : ""
        ...
        ${
            isCurrentUserReviewer(revision) &&
            revision.review_status === "completed"
                ? `
            <div class="detail-section bg-warning bg-opacity-10">
        ...
        <div class="detail-section">
            ...
            <p><strong>من طلب التعديل:</strong> ${
                revision.creator ? revision.creator.name : "غير محدد"
            }</p>
            ...
            ${
                revision.executor_user
                    ? `
                <div style="margin: 1rem 0; padding: 0.8rem; background: ${getExecutorBgColor(
                    revision.status
                )}; border: 3px solid ${getExecutorBorderColor(
                          revision.status
                      )}; border-radius: 8px;">
                    <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 0.5rem;">
                        ...
                        ${
                            (isCreator ||
                                revision.executor_user_id == currentUserId) &&
                            revision.revision_type === "project"
                                ? `
                    ...
                    ${(() => {
                        ...
                        if (executorDeadline) {
                            ...
                            if (diffMs < 0) {
                                deadlineColor = "#ef4444";
                                deadlineIcon = "fa-exclamation-triangle";
                                deadlineText = `فات الموعد بـ ${Math.abs(
                                    diffDays
                                )} يوم`;
                                deadlineBg = "#fef2f2";
                                deadlineBorder = "#ef4444";
                    ...
                    <div style="display: flex; align-items: center; justify-content: center; gap: 0.5rem; padding: 0.5rem; background: ${getExecutorStatusBg(
                        revision.status
                    )}; border: 2px solid ${getExecutorBorderColor(
                          revision.status
                      )}; border-radius: 6px;">
                        <i class="fas ${getExecutorStatusIcon(
                            revision.status
                        )}" style="color: ${getExecutorBorderColor(
                          revision.status
                      )}; font-size: 1.1rem;"></i>
                        <strong style="color: ${getExecutorBorderColor(
                            revision.status
                        )}; font-size: 0.95rem;">${getExecutorStatusText(
                          revision.status
                      )}</strong>
                    </div>
                </div>
            ...
            ${(() => {
                ...
                const reviewersHtml = reviewers
                    .map((reviewer, index) => {
                        const reviewerName = reviewer.user
                            ? reviewer.user.name
                            : window.allUsers?.find(
                                  (u) => u.id == reviewer.reviewer_id
                              )?.name || "مراجع " + reviewer.order;
                        const isCurrent =
                        ...
                        const currentBadge = isCurrent
                            ? '<span class="badge bg-success ms-2" style="font-size: 10px;">الدور الحالي</span>'
                            : "";
            ...
            ${
                !revision.responsible_user && !revision.executor_user
                    ? `
                <p><strong>المسند إليه:</strong> ${getAssignedUserName(
                    revision
                )}</p>
            `
                    : ""
        ...
        ${
            ...
            <div class="detail-section">
                ...
                ${
                    revision.project.code
                        ? `<small class="text-muted">${revision.project.name}</small>`
                        : ""
                }
            </div>
        `
        ...
        ${
            ...
            <div class="detail-section">
                ...
                `
                    ...
                    <div class="d-flex align-items-center gap-3">
                        <div>
                            <i class="${getAttachmentIcon(
                                revision.attachment_type
                        ...
                        <div class="flex-grow-1">
                            ...
                            <small class="text-muted">${formatFileSize(
                                revision.attachment_size
                            )}</small>
                        </div>
                        <div class="btn-group" role="group">
                            <a href="/task-revisions/${
                                revision.id
                            }/view" target="_blank" class="btn btn-outline-info btn-sm" title="عرض">
                                <i class="fas fa-eye me-1"></i>
                                عرض
                            </a>
                            <a href="/task-revisions/${
                                revision.id
                            }/download" class="btn btn-outline-primary btn-sm" title="تنزيل">
        ...
        ${
            ...
            <div class="detail-section">
                ...
                ${
                    revision.reviewed_at
                        ? `<small class="text-muted">تمت المراجعة في: ${formatDate(
                              revision.reviewed_at
                          )}</small>`
                        : ""
                }
        ...
        <div class="detail-section" id="transfer-history-section-${
            revision.id
        }">
...
async function loadTransferHistory(revisionId) {
    ...
    try {
        const response = await fetch(
            `/task-revisions/${revisionId}/transfer-history`,
            {
                method: "GET",
                headers: {
                    Accept: "application/json",
        ...
        if (result.success) {
            if (!result.history || result.history.length === 0) {
                $(contentId).html(
                    '<p class="text-muted text-center mb-0"><small>لم يتم نقل هذا التعديل من قبل</small></p>'
                );
                return;
            }
            ...
            result.history.forEach((transfer, index) => {
                const typeIcon =
                    transfer.assignment_type === "المنفذ"
                        ? "fa-hammer"
                        : "fa-user-check";
                const typeColor =
                    transfer.assignment_type === "المنفذ"
            ...
            $(contentId).html(
                '<p class="text-danger text-center mb-0"><small>حدث خطأ في تحميل سجل النقل</small></p>'
            );
        }
    } catch (error) {
        console.error("Error loading transfer history:", error);
        $(contentId).html(
            '<p class="text-danger text-center mb-0"><small>حدث خطأ في الاتصال بالخادم</small></p>'
        );
    }
}

function filterProjectsByStatus(status) {
    $(".projects-status-table tbody tr")
        .not(".projects-filter-tabs-row")
        .addClass("d-none");
